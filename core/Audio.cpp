...
Audio::Audio()
{
	...
	auto soundFiles = getFilesInDirectory(std::string(SOURCE_DIR) + "sounds");
	for (auto& f : soundFiles)
	{
		std::string name = getFileName(f, false);
		//printf("sound: \"%s\"\n", name.c_str());

		Mix_Chunk* chunk = Mix_LoadWAV((std::string(SOURCE_DIR) + "sounds/" + f).c_str());
		if (!chunk)
	...
	auto musicFiles = getFilesInDirectory(std::string(SOURCE_DIR) + "musics");
	for (auto& f : musicFiles)
	{
		std::string name = getFileName(f, false);
		//printf("music: \"%s\"\n", name.c_str());

		Mix_Music* music = Mix_LoadMUS((std::string(SOURCE_DIR) + "musics/" + f).c_str());
		if (!music)
			std::cerr << Mix_GetError() << "\n";
		else
			_musics[name] = music;
